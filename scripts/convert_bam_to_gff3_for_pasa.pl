...
perl convert_bam_to_gff3_for_pasa.pl -bam file.bam -fasta seq.fa -chr chr -out file.gff3 [-in_mono] [-clip_ends]
 ...
 -cautious    evaluate reads individually and ignore problematic ones (default: no) [optional]
              this is a way around an error generated by the Bio:DB::HTS library when fetching a small subset of spliced reads -
              note: extremely slow as each read has to be searched by name in the entire BAM file 
...
my $read_names;
if ($cautious_mode){
  $read_names = get_read_names($bam_file, $only_chr, $start, $end);
}
 
#Read BAM file
#Input: BAM file
#Output: array of Bio::DB::HTS:Alignment objects
my $alignments = read_bam_file($bam_file, $fasta_file, $cautious_mode);
print "N1=".scalar(@$alignments)." (Number of original reads) \n";


#Filter alignments
#Input: array of Bio::DB::HTS:Alignment objects
#Output: array of Bio::DB::HTS:Alignment objects
...
#Input: array of Bio::DB::HTS:Alignment objects, output file name
#Output: GFF3 file
...
sub get_read_names {
  my ($bam_file, $only_chr, $start, $end) = @_;
  my $loc = " ";
  $loc = $only_chr if $only_chr;
  $loc = "$only_chr:$start-$end" if ($only_chr and $start and $end);
  my @names = `samtools view $bam_file $loc | cut -f1`;
  chomp @names;
...
sub read_bam_file {
  my ($bam_file, $fasta_file, $cautious_mode) = @_;
  my @alignments;
  ...
  if ($cautious_mode){
    #This step makes the script very slow but error-proof
    foreach my $name (@$read_names){
      eval{					 
        my @feats = $sam->get_features_by_name(-name => $name);
        push(@alignments, @feats);
        #print "Found ".scalar(@feats)." alignments for $name\n";
      };
      if ($@){
        print "Got an error for $name\n";
      }
    }
    print "Found ".scalar(@alignments)." alignments\n";
  }
  else{
    @alignments = $sam->get_features_by_location( -seq_id => $only_chr,
                                                  -start  => $start,
                                                  -end    => $end
                                                );
  }
