...
my $same_intron_count;
my $max_reads_per_transcript;
my $keep_it_simple;
...
&GetOptions(
            ...
            'outfile=s'    => \$outfile,
            'same_introns!' => \$same_intron_count,
            'maxreads=i'   => \$max_reads_per_transcript,
...
to the splice site. If the score for any splice site is higher than a given threshold, the supporting read
will be considered not valid. If a transcript doesn't have any valid supporting read, it will be tagged for filtering.
...
perl check_splice_site_alignments.pl -gtf ANNOTATION_FILE -bamdir PATH_TO_BAM_DIR -fasta GENOME_FASTA_FILE -readsup READ_SUPPORT_FILE
 ...
 -outfile        output file
  ...
  -same_introns  only assess read alignments that have the same number of introns as the transcript (default = FALSE)
  -maxreads      maximum number of supporting reads to be checked per transcript (default = all) 
  -simple        stop checking a splice junction after finding a single valid supporting read (default = FALSE)
  -print_ali     print the read-genome alignment in the output file (default = FALSE)
...
while (<RSL>){
  ...
  if ($tids{$tid}){
    foreach my $read_name (split(/,/, $read_names)){
      push(@{$supporting_reads{$tid}{$file_name}}, $read_name);
    }
  }
}
close (RSL);
