...
Gene author name - only delete the current gene if this is its author.

=item B<-date>

Date in yyyy-mm-dd format - only delete the current gene if its modification date is the same or greater.
...

{
  ...
  GSI: foreach my $id (@stable_ids) {
      ...
      GENE: while (@$genes) {
            ...
            #If an author name is provided, skip this gene unless its author matches this
            if ($only_author and $cur_gene->gene_author->name ne $only_author){
              warn("Gene author is not $only_author\n");
              last GENE;
            }
            #If a modification date is provided, skip this gene unless its modification date matches this
            if ($date and Bio::EnsEMBL::Utils::ConversionSupport->date_format($cur_gene->modified_date, "%y-%m-%d") lt $date){
              warn("Last modification date is earlier than $date\n");
              last GENE;
            }
